...
describe('목표 E2E 테스트 - 모바일', () => {
  ...
  context('목표 삭제', () => {
    ...
    it('nav에서 목표를 삭제할 수 있어야 합니다.', () => {
      ...
      // nav에서 하나 클릭
      // 일부러 해당 페이지 방문. 삭제 후 대시보드로 이동하는지 확인.
      cy.contains('a', '새로운 목표 클릭 제출 테스트').click();
      ...
      cy.get('ul[aria-label="목표 전체 리스트"]')
        ...
        .each(($li) => {
          cy.wrap($li)
            .find('a')
            .should('not.contain', '새로운 목표 클릭 제출 테스트');
        });
